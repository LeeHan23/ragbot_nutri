...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        "Upload your .pdf or .docx files here",
    ...
    if st.button("Add Documents to Knowledge Base"):
        ...
        if not user_id:
            ...
            with st.spinner("Processing and training... This might take a while."):
                user_knowledge_path = os.path.join(USER_DATA_DIR, user_id, "knowledge")
                ...
                # Build the database, adding to existing knowledge
                build_database(user_id, status_callback=st.write, wipe_existing=False)
                
            st.success("Training complete! Your bot has learned from the new documents.")
    
    # --- NEW: Button to clear the knowledge base ---
    if st.button("Clear Knowledge & Start Fresh"):
        user_id = st.session_state.current_user_id
        if not user_id:
            st.error("Please start a user session first.")
        else:
            with st.spinner("Clearing knowledge base..."):
                user_knowledge_path = os.path.join(USER_DATA_DIR, user_id, "knowledge")
                user_db_path = os.path.join(CHROMA_BASE_PATH, user_id)
                
                if os.path.exists(user_knowledge_path):
                    shutil.rmtree(user_knowledge_path)
                if os.path.exists(user_db_path):
                    shutil.rmtree(user_db_path)
                
                # Also reset the chat history for a completely fresh start
                st.session_state.messages[user_id] = []
                st.session_state.user_data[user_id]["chat_history"] = []
                st.session_state.user_data[user_id]["visit_count"] = 1
                st.session_state.user_data[user_id]["intent_summary"] = "User is starting a new conversation."

            st.success(f"Knowledge base for user '{user_id}' has been cleared.")
...
if st.session_state.current_user_id:
    ...
    if not os.path.exists(user_db_path):
        st.warning("This bot has not been trained yet. Please upload documents and click 'Add Documents to Knowledge Base' in the sidebar to begin.")
    else:
